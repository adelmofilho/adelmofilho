---
title: "Mínimos quadrados via máxima verossimilhança"
...
data = matrix(0, ncol = 3, nrow = length(alpha)*length(beta))
i = 1
for (a in 1:length(alpha)) {
  
  for (b in 1:length(beta)) {
    
    data[i, ] = matrix(c(alpha[a], beta[b], 0), ncol = 3)
...
for (c in 1:length(alpha)^2) {
  
  erro = 0
  
  for (d in 1:dim(cars)[1]) {
    
    parcela = (cars[d,2]-(data[c,1]*cars[d,1] + data[c,2]))^2
...
scatterplot3d(data[,1],data[,2],data[,3], highlight.3d =TRUE, pch = 16, xlab = "Alpha", ylab = "Beta", zlab = "Erro Quadrático/10^7", box = TRUE)
```
